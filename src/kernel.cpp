...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZbtcaStake(spend));
    } else {
        ...
        CbtcaStake* btcaInput = new CbtcaStake();
        btcaInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(btcaInput);
    }
