...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZbtcrStake(spend));
    } else {
        ...
        CbtcrStake* btcrInput = new CbtcrStake();
        btcrInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(btcrInput);
    }
