...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 btca.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzbtcaReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzbtcaReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzbtcaReset.connect(boost::bind(NotifyzbtcaReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzbtcaReset.disconnect(boost::bind(NotifyzbtcaReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbtcaTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
