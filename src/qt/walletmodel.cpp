...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered btcr address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 btcr.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzbtcrReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzbtcrReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzbtcrReset.connect(boost::bind(NotifyzbtcrReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzbtcrReset.disconnect(boost::bind(NotifyzbtcrReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbtcrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
