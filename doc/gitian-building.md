...
*Setup instructions for a gitian build of btcr using a Debian VM or physical system.*
...
Clone the git repositories for btcr and gitian and then checkout the btcr version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the btcr repository
...
To build btcr (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the btcr repository.
...
```bash
    ...
    From https://github.com/forking-altcoins/btcr
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/btcr-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/btcr-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/btcr-build.assert
```
...
[btcr/gitian.sigs](https://github.com/forking-altcoins-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
